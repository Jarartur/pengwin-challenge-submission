...
# from nnunet.inference.nii2stl import save_STL_from_ITKImage
...
def remove_component_and_save(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
    ...
    if np.any([i != j for i, j in zip(np.array(current_shape[1:]), np.array(shape_original_after_cropping))]):
        ...
        seg_old_spacing = resample_data_or_seg(segmentation_softmax, shape_original_after_cropping, is_seg=False,
                                               ...
                                               order_z=interpolation_order_z, cupy=True)
    ...
    # =============================================================================
    #     added: remove_all_but_the_largest_connected_component
    # =============================================================================
    # if rm_components:
    #     volume_per_voxel = float(np.prod(properties_dict.get('spacing_after_resampling'), dtype=np.float64))
    #     seg_old_size_postprocessed,_,_ = remove_all_but_the_largest_connected_component(seg_old_size,None,volume_per_voxel,None)
    #     print("postprocessing: successfully removed all but the largest connected component")
    ...
    # elif save_stl==1:
    #     save_STL_from_ITKImage(seg_resized_itk, out_fname[:-4]+".stl")        
    # else: 
    #     sitk.WriteImage(seg_resized_itk, out_fname)
    #     save_STL_from_ITKImage(seg_resized_itk, out_fname[:-4]+".stl")
