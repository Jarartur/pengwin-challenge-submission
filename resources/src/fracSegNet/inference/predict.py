...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    :param list_of_lists: [[case0_0000.mha, case0_0001.mha], [case1_0000.mha, case1_0001.mha], ...]
    :param output_filenames: [output_file_case0.mha, output_file_case1.mha, ...]
...
def check_input_folder_and_return_caseIDs(input_folder, expected_num_modalities):
    ...
    files = subfiles(input_folder, suffix=".mha", join=False, sort=True)
    ...
    assert len(files) > 0, "input folder did not contain any images (expected to find .mha file endings)"

    # now check if all required files are present and that no unexpected files are remaining
    for c in maybe_case_ids:
        for n in range(expected_num_modalities):
            expected_output_file = c + "_%04.0d.mha" % n
    ...
    print("If they don't look right, make sure to double check your filenames. They must end with _0000.mha etc")
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
    ...
    output_files = [join(output_folder, i + ".mha") for i in case_ids]
    all_files = subfiles(input_folder, suffix=".mha", join=False, sort=True)
    list_of_lists = [[join(input_folder, i) for i in all_files if i[:len(j)].startswith(j) and
                      len(i) == (len(j) + 9)] for j in case_ids]

    if lowres_segmentations is not None:
        assert isdir(lowres_segmentations), "if lowres_segmentations is not None then it must point to a directory"
        lowres_segmentations = [join(lowres_segmentations, i + ".mha") for i in case_ids]
        assert all([isfile(i) for i in lowres_segmentations]), "not all lowres_segmentations files are present. " \
                                                               "(I was searching for case_id.mha in that folder)"
