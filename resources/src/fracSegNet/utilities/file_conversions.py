...
def convert_2d_image_to_nifti(input_filename: str, output_filename_truncated: str, spacing=(999, 1, 1),
    ...
    The image can have an arbitrary number of input channels which will be exported separately (_0000.mha,
    _0001.mha, etc for images and only .mha for seg).
    ...
    function will add the suffix (_0000) and file ending (.mha) for you.
    :param spacing:
    :return:
    """
    ...
    for j, i in enumerate(img):
        ...
        if not is_seg:
            sitk.WriteImage(itk_img, output_filename_truncated + "_%04.0d.mha" % j)
        else:
            sitk.WriteImage(itk_img, output_filename_truncated + ".mha")
...
def convert_3d_tiff_to_nifti(filenames: List[str], output_name: str, spacing: Union[tuple, list], transform=None, is_seg=False) -> None:
    ...
    for j, i in enumerate(filenames):
        ...
        if not is_seg:
            sitk.WriteImage(itk_img, output_name + "_%04.0d.mha" % j)
        else:
            sitk.WriteImage(itk_img, output_name + ".mha")
