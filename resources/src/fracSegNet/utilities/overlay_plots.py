...
def generate_overlays_for_task(task_name_or_id, output_folder, num_processes=8, modality_idx=0, use_preprocessed=True,
    ...
    if not use_preprocessed:
        ...
        identifiers = [i[:-4] for i in subfiles(join(folder, 'labelsTr'), suffix='.mha', join=False)]

        image_files = [join(folder, 'imagesTr', i + "_%04.0d.mha" % modality_idx) for i in identifiers]
        seg_files = [join(folder, 'labelsTr', i + ".mha") for i in identifiers]
...
def entry_point_generate_overlay():
    ...
    parser.add_argument('-modality_idx', type=int, default=0, required=False,
                        help="modality index used (0 = _0000.mha). Default: 0")
