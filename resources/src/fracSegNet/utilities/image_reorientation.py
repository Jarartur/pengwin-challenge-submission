...
def print_shapes(folder: str) -> None:
    for i in subfiles(folder, suffix='.mha'):
...
def reorient_to_ras(image: str) -> None:
    ...
    :return:
    """
    assert image.endswith('.mha')
    origaffine_pkl = image[:-4] + '_originalAffine.pkl'
...
def reorient_all_images_in_folder_to_ras(folder: str, num_processes: int = 8):
    ...
    nii_files = subfiles(folder, suffix='.mha', join=True)
...
if __name__ == '__main__':
    ...
    test_img = '/home/fabian/data/la_005_0000.mha'
    test_img_reorient = test_img[:-4] + '_reorient.mha'
    test_img_restored = test_img[:-4] + '_restored.mha'
