...
if __name__ == "__main__":
    ...
    parser.add_argument('--predictions', type=Path, help='Prediction image')
    parser.add_argument('--gt', type=Path, help='Ground Truth image')
    args = parser.parse_args()

    results = []
    metrics_multi_case = {"fracture_iou": [],
                          "fracture_hd95": [],
                          "fracture_assd": [],
                          "anatomical_iou": [],
                          "anatomical_hd95":[],
    ...
    for pred in tqdm(args.predictions.rglob("*.pred.nii.gz")):
        pred_volume = load_image_file(location=pred)
        spacing, gt_volume = load_gt_label_and_spacing((args.gt/pred.name.removesuffix("".join(pred.suffixes))).with_suffix(".mha"))
        metrics_single_case = evaluate_3d_single_case(gt_volume, pred_volume, spacing, verbose = True)
        results.append({pred.name: metrics_single_case})
        for key, val in metrics_single_case.items():
            metrics_multi_case[key].append(val)
    
    for key, val in metrics_multi_case.items():
        metrics_multi_case[key] = np.mean(val)

    with open(args.predictions/"metrics_multi_case.json", 'w') as file:
        json.dump(metrics_multi_case, file, indent=4)
        
    with open(args.predictions/"results.json", 'w') as file:
